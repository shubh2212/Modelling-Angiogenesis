...
void update_concentration( int i,int j ) {
	//forward difference method to calculate concentration at (n+1)th time level
	 concentration[i][j][time_] = -dt*neta*(density[i][j][time_-1]*concentration[i][j][time_-1]) + concentration[i][j][time_-1];
}


void update_f( int i, int j){
	//forward difference method to calculate "f" at (n+1)th time level
	f[i][j][time_] = f[i][j][time_-1] + dt*( beta*density[i][j][time_-1] - gamma*density[i][j][time_-1]*f[i][j][time_-1] );
}
...
void matmul2(double df2[][col],double df4[][col], double df5[][col],double df6[][col], double df8[][col], double t[][col], double at[][col]){
	at[0][0] = df5[0][0]*t[0][0] + df6[0][0]*t[1][0]+df8[0][0]*t[0][1];
	at[row-1][0] = df5[row-1][0]*t[row-1][0] + df4[row-1][0]*t[row-2][0]+df8[row-1][0]*t[row-1][1];
	at[0][col-1] = df5[0][col-1]*t[0][col-1] + df4[0][col-1]*t[1][col-1]+df2[0][col-1]*t[0][col-2];
	at[row-1][col-1] = df5[row-1][col-1]*t[row-1][col-1] + df4[row-1][col-1]*t[row-2][col-1]+df2[row-1][col-1]*t[row-1][col-2];
	int i,j;
	for(i=1;i<row-1;i++){
		at[i][0] = df5[i][0]*t[i][0] + df6[i][0]*t[i+1][0] +df4[i][0]*t[i-1][0]+ df8[i][0]*t[i][1];
		at[i][col-1] = df5[i][col-1]*t[i][col-1] + df6[i][col-1]*t[i+1][col-1] + df4[i][col-1]*t[i-1][col-1] + df2[i][0]*t[i][col-2];
	}
	for(j=1;j<col-1;j++){
		at[0][j] = df5[0][j]*t[0][j]+df8[0][j]*t[0][j+1] + df2[0][j]*t[0][j-1] + df6[1][j]*t[1][j];
		at[row-1][j] = df5[row-1][j]*t[row-1][j]+df8[row-1][j]*t[row-1][j+1] + df2[row-1][j]*t[row-1][j-1] + df4[row-1][j]*t[row-2][j];
	}
	for(j=1;j<col-1;j++){
		for(i=1;i<row-1;i++){
